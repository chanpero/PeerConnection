...
bool ReceiverReport::Parse(const CommonHeader& packet) {
  ...
  // chanper: handle profile-specific data
  uint32_t myData = ByteReader<uint32_t>::ReadBigEndian(packet.payload() +
                                      ReceiverReport::kRrBaseLength + ReportBlock::kLength * report_blocks_count);
  RTC_LOG(INFO) << __FUNCTION__ << ": " << myData;

  return true;
}

size_t ReceiverReport::BlockLength() const {
  // chanper: add four bytes profile-specific data
  return kHeaderLength + kRrBaseLength +
         report_blocks_.size() * ReportBlock::kLength + kRrBaseLength;
}
...
bool ReceiverReport::Create(uint8_t* packet,
  ...
  *index += kRrBaseLength;
  ...
  // chanper: add profile-specific extensions
  ByteWriter<uint32_t>::WriteBigEndian(packet + *index, uint32_t(0x00000004));
  *index += kRrBaseLength;
  return true;
}
